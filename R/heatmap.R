#' Heatmapscript use heatmapscriptnew if error
#'
#'
...
heatmapscript <- function(data=NULL,selcol=NULL, refno= 1, refgene= data, name = NULL, destination = .., clcol= TRUE){
  ...
  # subsetted_filein_avg$SYMBOL <- make.names(subsetted_filein_avg$SYMBOL, unique=TRUE)
  heat_map_matrix <- data.frame(subsetted_filein_avg[,"SYMBOL"],subsetted_filein_avg[,selcol])
  ...
  set.seed(2019-03-21)
  heat<- pheatmap(center_raw_mat_logged_matrix, scale="row",  #double <<- spits asd out into global enviro
