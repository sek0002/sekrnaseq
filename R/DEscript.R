...
DE_analysis <- function(countdata,sampleinfo, thresh=0.5, keep, group_param, exp_name, design, DEgroup, cont.matrix, species, isEnsembl="FALSE", qlftest=T, plots=F, human_ensembldb= "EnsDb.Hsapiens.v75", mouse_ensembldb= "EnsDb.Mmusculus.v102") {
  ...
  if(plots==T){
    for (x in colnames(cont.matrix)) {
      ...
      limma.res<- limma.res[order(row.names(limma.res)),]
      summa.limmafit.reorder <- summa.limmafit[order(row.names(summa.limmafit)),]
      limmafit.cont$limma.res <- limma.res
      summary(summa.limmafit)
      table(summa.limmafit)

      limmafit.cont$limma.res$Threshold <- ifelse(limmafit.cont$limma.res$adj.P.Val <=0.05 & limmafit.cont$limma.res$logFC >0,"Up",
                                                  ifelse(limmafit.cont$limma.res$adj.P.Val <=0.05 & limmafit.cont$limma.res$logFC <0, "Down",
                                                         "notsig"))
  ...
  plotBCV(y1)
  edgeRfit <- glmQLFit(y1, design) ### no robust= like DEGUST
  plotQLDisp(edgeRfit)
  qlf_file<- list()
  ...
  if(plots==T){
    ...
    for (x in colnames(cont.matrix)){
      ...
      qlf <- glmQLFTest(edgeRfit, contrast=cont.matrix[,x])
      summary(decideTests(qlf))
      plotMD(qlf)

      EdgeR.res <- topTags(qlf, n=Inf, sort.by='none')$table
      EdgeR.summa <- decideTests(qlf, p.value=0.05)
      EdgeR.summa.reorder<- EdgeR.summa[match(rownames(EdgeR.res), rownames(EdgeR.summa)),]
      ...
      maplotniceqlf(qlf$EdgeR.res, xaxis= "logCPM", yaxis= "logFC", paste0(x,"_qlf_MA",sep=""), exp_name)

      nicevolcqlf(qlf$EdgeR.res, xaxis= "logFC", yaxis= "FDR", paste0(x,"_qlf_volc",sep=""), exp_name)

      glMDPlot(qlf, main=paste(x), groups= sampleinfo[,group_param],
               status = EdgeR.summa.reorder, counts = y1$counts, side.main="SYMBOL",
               anno=qlf$EdgeR.res,path= paste0("Volcano/",exp_name,"/interactive"), folder=paste0(x,"_qlf_MA"))
      ...
      glXYPlot(x=qlf$EdgeR.res$logFC, y=-log10(qlf$EdgeR.res$FDR),
               ...
               anno=qlf$EdgeR.res, side.main="SYMBOL", path= paste0("Volcano/",exp_name,"/interactive"),folder=paste0(x,"_qlf_volc"))


      qlf_file[x]<- list(qlf)

      write.table(qlf$EdgeR.res, file=paste0("DE/",exp_name,"/qlf_output_",x,".tsv",sep=""), sep="\t", row.names = F)
    }
  }else{
    for (x in colnames(cont.matrix)){
